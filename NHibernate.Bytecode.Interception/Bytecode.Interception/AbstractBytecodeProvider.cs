...
namespace NHibernate.Bytecode.Interception
{
    public abstract class AbstractBytecodeProvider : IBytecodeProvider, IInjectableProxyFactoryFactory, IInjectableCollectionTypeFactoryClass
    {
        ...
        public AbstractBytecodeProvider()
            : this(new EmptyBytecodeProviderInterceptor())
        { }

        public AbstractBytecodeProvider(IBytecodeProviderInterceptor interceptor)
        {
            if (interceptor == null)
            {
                new ArgumentNullException("interceptor");
            }

            this.interceptor = interceptor;
            this.objectsFactory = new ActivatorObjectsFactory(interceptor);
            this.collectionTypeFactoryClass = typeof(Type.DefaultCollectionTypeFactory);
        }
        #endregion
        ...
        public virtual IProxyFactoryFactory ProxyFactoryFactory
        {
            get
            {
                ...
                return new DefaultProxyFactoryFactory(interceptor);
            }
        }
        ...
        #region Interceptor
        public void SetInterceptor(IBytecodeProviderInterceptor interceptor)
        {
            this.interceptor = interceptor;
        }
        #endregion
